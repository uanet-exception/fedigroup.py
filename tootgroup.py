...
def main():
    ...
    my_config_dir, my_config_file = setup_configuration_path(
        "tootgroup.py", "tootgroup.conf")
    ...
    my_config = parse_configuration(
        my_config_dir, my_config_file, my_group_name)

    # If the "catch-up" commandline argument has been given, reset the last
    # seen ID so that tootgroup.py only updates its according config value
    # without retooting anything.
    ...
    for member in masto.account_followers(my_account["id"]):
    ...
    while get_more_notifications:
        ...
        if notification_count == 0:
            if len(get_notifications) > 0:
                ...
                # If there have not been any notifications yet, set value to 0
                latest_notification_id = 0
                print("Nothing to do yet! Start interacting with your group account first.")  # NOQA
                get_more_notifications = False
        else:
            # leave the while loop if there are no more notifications fetched
            if len(get_notifications) == 0:
                get_more_notifications = False
        ...
        if my_config[my_group_name]["last_seen_id"] == "catch-up":
            my_config[my_group_name]["last_seen_id"] = str(latest_notification_id)  # NOQA
            write_new_config = True
            print("Caught up to current timeline. Run again to start group-tooting.")  # NOQA
...
def media_toot_again(orig_media_dict, mastodon_instance):
    ...
    for media in orig_media_dict:
        ...
        # basename still includes a "?" followed by a number after the file's name.  # NOQA
        # Remove them both.
        filename = filename.split("?")[0]
        # separate filename and extension
        filename, file_ext = os.path.splitext(filename)
        f_temp = tempfile.NamedTemporaryFile(suffix=file_ext, delete=False, mode="w+b")  # NOQA
        try:
            ...
            print(e)
            print("")
            print("tootgroup.py will continue but media files might not get reposted!")  # NOQA
            print("##################################################################\n")  # NOQA
        finally:
...
def new_credentials_from_mastodon(group_name, config_dir, config):
    ...
    try:
        ...
        print("")
        print("\n##################################################################")  # NOQA
        print("The Mastodon instance URL is wrong or the server does not respond.")  # NOQA
...
def parse_configuration(config_dir, config_file, group_name):
    ...
    does also take care of a first run situation where nothing is set up yet
    and in that way act as an installer!

    parse_configuration should always return
    a complete and usable configuration"""
